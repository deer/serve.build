...
```
serve.build/
├── serve-foundation/           Core contracts: Exchange, Handler, Router, Middleware, errors, options, Validate
...
├── serve-testing/              TestServer (ephemeral port), TestRequest, TestResponse + WebSocket/SSE clients
│
...
├── serve-mcp/                  Model Context Protocol (streamable HTTP + SSE; session management)
...
├── serve-auth/                 Authentication middleware; Bearer/Basic/ApiKey strategies; Principal SPI
├── serve-ratelimit/            Token-bucket rate limiting; per-IP by default; 429 + Retry-After headers
├── serve-session/              Cookie-based server-side sessions; pluggable SessionStore SPI
├── serve-form/                 url-encoded + multipart form body parsing
│
...
│
├── serve-example/              Full demo app: REST + GraphQL + WebSocket + templates + health
│
...
**Purpose:** Core abstractions — HTTP exchange contracts, routing DSL, middleware composition, error hierarchy, concurrency utilities, server configuration options, and request validation. No I/O; pure interfaces and thin value types.
...
| `RouterBuilder.java` | Fluent builder: `.get/.post/.put/.delete/.route/.group/.middleware/.build()` |
...
| `Validate.java` | Declarative request validation throwing `BadRequestException` (400); `notNull`, `notBlank`, `present`, `that` |
...
- `RouterBuilder.group(configure)` — scoped middleware for a subset of routes without path prefix stripping
...
**Purpose:** Integration test helpers — real `HttpTransport` on an ephemeral port, fluent request builder, fluent response assertions, and protocol-specific clients for WebSocket and SSE.

**Key files:**

| File | Purpose |
|------|---------|
| `TestServer.java` | `AutoCloseable`; binds `127.0.0.1:0`; factory overloads: `of(Handler)`, `of(Router)`, `of(RouterBuilder)`, `of(Handler, ErrorHandler)`; `connectWebSocket(path)`, `sse(path)` |
...
| `TestWebSocket.java` | JDK WebSocket client with blocking `nextText()`/`nextBinary()` queue; `assertClosed()` |
| `TestSseStream.java` | Streaming SSE client; `collect(n, timeout)` for N events, `collectAll(timeout)` for stream close |
| `TestSseEvent.java` | Record: `data`, `event`, `id` — parsed SSE event |
...
- `TestWebSocket` uses JDK `HttpClient.newWebSocketBuilder()` — requires `java.net.http` module access

**Dependencies:** `build.serve.foundation` (transitive), `build.serve.transport.http`, JUnit 5 (transitive), AssertJ (transitive), Jackson, `java.net.http`
...
**Purpose:** Model Context Protocol server — JSON-RPC 2.0 over HTTP POST with streamable HTTP transport; session management; optional SSE response; tool call event publishing.

**Key files:**

| File | Purpose |
|------|---------|
| `McpServer.java` | Builder + `handler()` factory; implements `initialize`, `ping`, `tools/list`, `tools/call`; issues `Mcp-Session-Id`; responds as JSON or SSE |
...
| `ToolCallEvent.java` | Event published after each tool call; carries tool name + result |

**Protocol (MCP streamable HTTP, spec 2025-03-26):**
```
POST /mcp  (no Accept: text/event-stream)
  → JSON response: {"jsonrpc":"2.0","id":1,"result":{...}}

POST /mcp  (Accept: text/event-stream)
  → SSE response: event: message\ndata: {"jsonrpc":"2.0","id":1,"result":{...}}\n\n

POST /mcp  (Mcp-Session-Id: <unknown>)
  → 404 Session not found

initialize → issues Mcp-Session-Id header on response
...
- Session IDs stored in-memory (`ConcurrentHashMap`) — not durable across restarts
- Protocol version hardcoded: `"2025-03-26"`
- No batched JSON-RPC support

**Dependencies:** `build.serve.foundation` (transitive), `build.serve.sse`, `com.fasterxml.jackson.databind` (transitive), `build.base.flow`
...
`RouterBuilder.group(configure)` lets you apply middleware to a subset of routes without a path prefix:
```java
...
**Purpose:** Authentication middleware with pluggable strategies and a `Principal` SPI. On auth failure, sends 401 with a `WWW-Authenticate` challenge and short-circuits the handler chain. On success, binds the principal to `RequestContext.PRINCIPAL` via `ScopedValue`.

| File | Purpose |
|---|---|
| `AuthMiddleware.java` | Middleware; delegates to a configured `AuthStrategy`; binds `Principal` via `ScopedValue.where(RequestContext.PRINCIPAL, ...)` |
| `AuthStrategy.java` | `@FunctionalInterface`: `Optional<Principal> authenticate(Request)`; `challenge()` default; `anyOf(...)` combinator |
| `Principal.java` | Identity interface: `id()`, `name()` |
| `SimplePrincipal.java` | Concrete record implementing `Principal` |
| `AuthContext.java` | Static `current()` — reads `RequestContext.PRINCIPAL` if bound |
| `BearerTokenStrategy.java` | Validates `Authorization: Bearer <token>`; accepts `Function<String, Optional<Principal>>` validator |
| `BasicAuthStrategy.java` | Validates `Authorization: Basic <base64(user:pass)>`; configurable realm |
| `ApiKeyStrategy.java` | Reads key from a header (`fromHeader`) or query param (`fromQueryParam`) |
...
// Access in handler:
AuthContext.current().ifPresent(p -> response.send("Hello, " + p.name()));
```

**Gotcha:** `AuthMiddleware` stores the principal as a raw `Object` under `RequestContext.PRINCIPAL`; `AuthContext.current()` casts it to `Principal` — ensure no other code stores a non-`Principal` under that key.

**Dependencies:** `build.serve.foundation`

---

### serve-ratelimit

**Purpose:** Token-bucket rate limiting middleware. Requests within the limit pass through; excess requests receive 429 with `Retry-After`, `X-RateLimit-Limit`, and `X-RateLimit-Remaining` headers.

| | |
|---|---|
| **Factory** | `RateLimitMiddleware.builder().limit(100).per(Duration.ofMinutes(1)).build()` |
| **Default key** | First IP in `X-Forwarded-For` header (or `"unknown"` if absent) |
| **Custom key** | `.keyExtractor(req -> req.header("X-Api-Key").orElse("anon"))` — rate limit by user, API key, etc. |
| **Algorithm** | Token bucket with continuous refill; `synchronized` per-bucket for thread safety |
| **Gotcha** | Buckets accumulate unboundedly — no eviction of idle keys; in-memory only (not distributed) |
...
**Purpose:** Cookie-based server-side sessions with a pluggable `SessionStore` SPI. Each request loads or creates a session, binds it via `ScopedValue`, and saves/deletes it after the handler chain. Integrates with `serve-auth`: a principal stored under `SessionMiddleware.PRINCIPAL_KEY` is automatically bound to `RequestContext.PRINCIPAL`.

| File | Purpose |
|---|---|
| `SessionMiddleware.java` | Middleware; cookie read/write; delegates load/save to `SessionStore` |
| `Session.java` | Interface: `id()`, `get/set/remove`, `invalidate()`, `isNew()`, `isInvalidated()`, `attributes()` |
| `SessionStore.java` | SPI: `load(id)`, `save(session)`, `delete(id)` |
| `InMemorySessionStore.java` | Default implementation; `ConcurrentHashMap`; no TTL eviction |
| `MapSession.java` | `ConcurrentHashMap`-backed `Session` implementation |
| `SessionContext.java` | Static `current()` — reads `SESSION` `ScopedValue` |
...
SessionMiddleware.builder()
    ...
    .maxAge(Duration.ofDays(30))  // persistent cookie
    .build()

// In handler:
SessionContext.current().ifPresent(s -> {
    s.set("userId", userId);
    s.set(SessionMiddleware.PRINCIPAL_KEY, principal);  // propagates to AuthContext
});
```

**Cookie defaults:** `HttpOnly`, `SameSite=Lax`, `Path=/`; `Secure` is opt-in.

**Gotcha:** `InMemorySessionStore` has no TTL eviction — sessions accumulate forever in long-running servers. For production, implement `SessionStore` with Redis or database backing.
...
**Purpose:** Parses `application/x-www-form-urlencoded` and `multipart/form-data` request bodies. No middleware; parse on demand from `Request`.

| File | Purpose |
|------|---------|
| `FormData.java` | Parses url-encoded bodies; `from(request)` or `parse(string)`; multi-value field support |
| `MultipartData.java` | Parses multipart bodies; extracts boundary from `Content-Type`; `get(name)`, `getAll(name)`, `parts()` |
| `FormPart.java` | Single multipart part: `name()`, `filename()`, `contentType()`, `bytes()`, `value()` |
...
var avatar = multipart.get("avatar");  // Optional<FormPart>
```

**Gotchas:**
- `MultipartData.from()` calls `request.bodyAsStream().readAllBytes()` — reads entire body into memory
- Multipart parser uses ISO-8859-1 for byte/string conversion; binary file parts are correctly handled but text parts with non-ASCII chars may need explicit charset conversion
- `MultipartData.from()` throws `IllegalArgumentException` if `Content-Type` header is missing or has no boundary
...
```
GET  /                          → full page (base template)
POST /tasks                     → create task, return partial
PUT  /tasks/{id}/toggle         → toggle, return partial
...
- **`@FunctionalInterface` for extension points:** `Handler`, `Middleware`, `DataFetcher`, `HealthCheck`, `SseHandler`, `WebSocketHandler`, `TemplateEngine`, `Template`, `AuthStrategy`
...
| `serve-mcp` | In-memory session store — sessions lost on restart; no TTL eviction |
| `serve-session` | `InMemorySessionStore` has no TTL eviction — sessions accumulate forever |
| `serve-ratelimit` | No bucket eviction — idle key buckets accumulate in memory |
| `serve-form` | `MultipartData.from()` reads entire body into memory; throws if `Content-Type` is missing |
| `serve-auth` | `AuthContext.current()` casts raw `Object` from `RequestContext.PRINCIPAL` to `Principal` — unsafe if another module stores a non-`Principal` there |
...
**To add auth to a subset of routes:**
```java
RouterBuilder.create()
...
**To validate request input:**
```java
var title = Validate.notBlank("title", body.title());
var id    = Validate.present("id", exchange.request().pathParam("id"));
```
...
try (var server = TestServer.of(myRouter)) {
    server.get("/api/items").send().assertStatus(200).assertBodyContains("items");
    // WebSocket:
    try (var ws = server.connectWebSocket("/ws")) {
        ws.sendText("hello");
        assertThat(ws.nextText()).isEqualTo("hello");
    }
    // SSE:
    try (var sse = server.sse("/events")) {
        var events = sse.collect(3, Duration.ofSeconds(5));
...
// File upload
var multipart = MultipartData.from(exchange.request());
var file = multipart.get("avatar").map(FormPart::bytes).orElseThrow();
```
...
// In handler:
SessionContext.current().ifPresent(s -> s.set("userId", userId));
```
